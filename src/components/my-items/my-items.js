...
const MyItems = () => {
    ...
    return (
        <>
            <h1 class="text-white bg-info text-center mb-4 mt-4 "> My Items List Screen! </h1>
                    ...
                    (
                        itemsList.map((item, index) => {
                            if (item.userId == user.email) {
                                return (
                                    <h3  class="list-group-item list-group-item-info text-center text-uppercase  " key={index}> {item.itemName} </h3>
                                );
                            }
                        })
                    )
